...
unless ( -f "/html/graphs.php" ){
  system("tar", "xf", "/zabbix.tar.gz", "-C", "/");
}

system("chown docker.docker -R /html/conf");

my $conf="/html/conf/zabbix.conf.php";
system("chmod", "640", "-R", "$conf") if ( -f $conf );
